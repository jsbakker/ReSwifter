...
/// Creates a new Driver instance. Caller must call webcpp_driver_destroy when done.
WebCppDriverRef webcpp_driver_create(void);

/// Destroys a Driver instance.
void webcpp_driver_destroy(WebCppDriverRef driver);

/// Prints help text to stderr. mode is 'L' for languages or 'D' for default usage.
void webcpp_driver_help(char mode);

/// Parses a command-line-style option string (e.g. "-l", "--line-numbers", "-c=scheme").
/// Returns true if the option was valid.
...
/// Detects the language for the given filename and returns a description string.
...
///   filename – a representative filename used to detect the language (e.g. "example.py")
///   options  – a null-terminated array of option strings (e.g. {"-l", "-s", NULL}), or NULL
